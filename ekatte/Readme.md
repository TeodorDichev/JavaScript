...
### Бекенд сървър
Реализиран с **Express.js** и използва библиотеката **CORS**, за да разреши безопасно извикване на API от фронтенд сървъра на различен порт (`http://localhost:8000`).  

Основният маршрут за търсене е дефиниран в **router.js** (или `search.js`) като **Express Router**. Той обработва **GET** заявки с параметър `q` и връща **JSON** от базата данни, използвайки **JOIN** между таблиците `settlement`, `mayorality`, `municipality` и `region`.  
